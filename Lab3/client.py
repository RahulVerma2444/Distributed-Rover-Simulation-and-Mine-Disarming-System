    ...
    with grpc.insecure_channel("localhost:50051") as channel:
        ...
        for command_reply in command_replies: #run through each of the rover commands
            ...
            if command == 'R': #rotate to right
                ...
                #share mine coordinates, id, serial number to demine_queue
                #---------------------------------------------------------
                ...
                Demine_Queue = {
                    'MineCoordinates' : f"({i}, {j})",
                    'pin' : str(pin),
                    'serialNumber' : mine_reply_formatted
                }

                channel.basic_publish(
                    exchange = 'Demine_Queue',
                    routing_key='Demine_Queue.notify',
                    body=json.dumps(Demine_Queue)
                )

                print(f"[x] Sent MineCoordinates, Id, and SerialNumber")
                connection.close()
        ...
        if(success == 1): #if traversed through commands successfully
            print("Traversed through commands successfully")
