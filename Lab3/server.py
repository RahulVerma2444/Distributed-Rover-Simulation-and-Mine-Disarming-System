...
def subscribeDefused(): #Subscriber to Defused Mines----------------------------------
    print("Defused_Mines2")
    connection2 = pika.BlockingConnection(pika.ConnectionParameters(host='localhost'))
    channel2 = connection2.channel()
    
    channel2.exchange_declare(exchange='Defused_Mines2', exchange_type='direct')

    queue = channel2.queue_declare(queue='Defused_Mines2_notify')
    ...
    def callback(ch, method, properties, body):
        ...
        print('[x] Information from Defused_Mines2:')
        print(f" [x] Pin: {body}")
